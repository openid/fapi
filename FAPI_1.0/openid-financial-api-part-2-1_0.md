...
not exposed to the attacker, which therefore cannot authenticate towards the token endpoint of the authorization server. However, there is still the potential for the attacker to inject the phished code into an honest client’s flow.
